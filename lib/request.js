  ...
  .then(response => {
    ...
    if(this.config.resultParsers){
      if(response.result instanceof Array){
            ...
            if(this.config.params.group_by){
              ...
              response.result.forEach((val) => {
                val.value.forEach((res) => {
                  this.config.resultParsers.forEach((func) => {res.result = func(res.result)})
                })
              })
            }
            else {
              //interval result
              response.result.forEach((val) => {
                this.config.resultParsers.forEach((func) => {val.value = func(val.value)})
              })
            }
          }
          else {
        ...
        //simple result
        this.config.resultParsers.forEach((func) => {response.result = func(response.result)})
      }
    }
