...
KeenLibrary.prototype.queryLocal = (query) => {
  return new Promise((resolve, reject) => {
    ...
    query.main_filters = [...query.filters];
    ...
    const parseTimeframe = ({ timeframe, returnPartial = false, commonTime }) => {
      if (timeframe.start) {
        // absolute
        return { start: moment.utc(timeframe.start), end: moment.utc(timeframe.end)};
      }
      ...
      if (!returnPartial) {
        return { start: moment.utc(commonTime).subtract(unitsNumber, units), end: moment.utc(commonTime)};
      }
    ...
    if (query.timeframe) {
      ...
      if (query.timeframe.start) {
        ...
        const timestamp_to_value_this = moment.utc().startOf(timestamp_to.units.slice(0, timestamp_to.units.length -1));
    ...
    const lastRow = query.data.result && query.data.result[query.data.result.length - 1];
    const lastRowTimeStamp = lastRow && lastRow.keen.timestamp;
    const intervalFilters = [];
    if (query.interval){
      const startTimestamp = query
        .filters.find(item => item.property_name === 'keen.timestamp'
          && item.operator === 'gt').property_value;
      ...
      if (query.interval === 'daily') {
        ...
        while(endTimestamp > currentEndTimestamp) {
          let timeframe = {
            start: currentEndTimestamp.startOf(units).toISOString(),
            end: currentEndTimestamp.add(1, units).toISOString()
          };
          promisesArray.push(
            KeenLibrary.prototype.queryLocal({
              ...query,
              timeframe: { ...timeframe },
              interval: null,
              filters: [...query.main_filters],
              extendResults: {
                timeframe: { ...timeframe }
              }
            })
        ...
        switch(filter.operator){
          ...
          case 'gt':
            if (filter.property_name === 'keen.timestamp') {
              return moment.utc(d) > filter.property_value;
            }
            return d > filter.property_value;
          break;
          case 'gte':
...
function parseLocalResponse({ result, parser = {}, query = {} }){
  ...
  return { result: resultParsed, ...query.extendResults };
}
