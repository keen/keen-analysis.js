...
KeenLibrary.prototype.query = function(a, b, options = {}){
  let requestQuery = {};
  if (a && b && typeof b === 'string') {
    ...
    requestQuery = this
      .get(this.url('queries', a, b), options)
      .auth(this.readKey());
    return requestQuery.send();
  }
  else if (a === 'dataset' && typeof b === 'object') {
    requestQuery = this
      .get(this.url('datasets', b.name, 'results'), options)
      .auth(this.readKey());
    ...
    requestQuery = this
      .post(this.url('queries', a), options)
      .auth(this.readKey());
